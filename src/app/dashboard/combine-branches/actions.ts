...
export async function fetchBranchesForCombination(branchIds: string[]) {
  const filteredIds = branchIds.map(id => id.trim()).filter(id => id.length > 0);
  if (filteredIds.length === 0) {
    throw new Error("No valid branch IDs provided");
  }
  ...
  if (branches.length === 0) {
    throw new Error("No branches found matching the provided IDs.");
  }
  ...
  const branchSummaries = branches.map(branch => {
    ...
    items.forEach((item: any) => {
      ...
      item.branchName = branch.branchName || "Unnamed Branch";
    });
    allItems.push(...items);

    const totalQty = items.reduce((sum: number, i: any) => sum + i.qty, 0);
    const totalProfit = items.reduce((sum: number, i: any) => sum + (i.finalPrice - i.avgPrice) * i.qty, 0);
    const totalValue = items.reduce((sum: number, i: any) => sum + i.finalPrice * i.qty, 0);

    return {
      id: branch.id,
      name: branch.branchName || "Unnamed Branch",
      from: branch.fromCompany,
      to: branch.toCompany,
      totalQty,
      totalProfit,
      totalValue
...
export async function updateCombinedBranchOverrides(
  branchIds: string[],
  updates: Array<{ sku: string; productName: string; grade: string; price: number }>
) {
  const filteredIds = branchIds.map(id => id.trim()).filter(id => id.length > 0);
  const branches = await prisma.invoiceExport.findMany({
    where: {
  ...
  for (const branch of branches) {
    ...
    for (const update of updates) {
      let key = "";
      if (mode === "mixed") {
        key = update.productName;
      } else if (mode === "separate") {
        key = update.sku;
      } else if (mode === "premium-mixed") {
        key = update.grade === "Premium" ? update.sku : `${update.productName}_NON_PREMIUM`;
      }

      if (key) {
        config.rowOverrides[key] = update.price;
        modified = true;
      }
    }
    ...
    if (modified) {
      await prisma.invoiceExport.update({
        where: { id: branch.id },
        data: {
          markupConfig: JSON.stringify(config)
        }
      });
    }
  }
